...
##################################################################
#
# Processing Script for Total Housing Units by County
...
dataset <- merge(counties, dataset, by = "FIPS", all.x=T)

dataset$`Occupancy Status` <- factor(dataset$`Occupancy Status`, levels = c("Total", "Vacant", "Occupied"))

#set final column order
dataset <- dataset %>% 
  select(County, FIPS, Year, `Occupancy Status`, `Measure Type`, Variable, Value) %>% 
  arrange(County, Year, `Occupancy Status`, `Measure Type`, Variable)

# Write to File
write.table(
    dataset,
    file.path(getwd(), "data", "total-housing-units-by-county-2019.csv"),
    sep = ",",
    row.names = F,
    na = "-9999"
)
